...
define( ["qlik", "jquery", "text!./style.css"], function ( qlik, $, cssContent ) {
	...
	function createRows ( rows, dimensionInfo ) {
		...
		rows.forEach( function ( row ) {
			...
			row.forEach( function ( cell, key ) {
				...
				if ((key==3)||(key==6)||(key==9)||(key==12)||(key==15)||(key==16)) {
	...
	return {
		...
		paint: function ( $element, layout ) {
			//layout.qOtherTotalSpec.qOtherMode = 2;
			...
			hypercube.qMeasureInfo.forEach( function ( cell ) {
				switch (contcol) {
				  ...
				  case 6:
					...
					break;
				  case 7:
					html += '<th colspan="3" class="rec"><h3>RECEITA</h3></th>';
					contcol=contcol+1;
					break;
				  case 8:
				  	contcol=contcol+1;
			...
			hypercube.qMeasureInfo.forEach( function ( cell ) {
				if((contcol==1)||(contcol==2)||(contcol==3)) {
				  ...
				  html += '<th class="rec">' + cell.qFallbackTitle + '</th>';
				  contcol=contcol+1;
				} else
				  if(contcol==10 || contcol==11 || contcol==12) {
				  html += '<th class="mc">' + cell.qFallbackTitle + '</th>';
				  contcol=contcol+1;
				} else
				  if(contcol==13 || contcol==14 || contcol==15 || contcol==16 || contcol==17) {
			...
			hypercube.qGrandTotalRow.forEach( function ( cell ) {
				html += '<th';
				if ((contcol==3)||(contcol==6)||(contcol==9)||(contcol==12)||(contcol==15)||(contcol==16)) {
					if( parseFloat(cell.qText) < 0 ) {
						html += " style='color: red;'";
					} else if ( parseFloat(cell.qText) > 0 ) {
						html += " style='color: green;'";
					}
				}
